...
export async function GET(req){
    ...
    try {  
        ...
        const status = (searchParams.get('status') != '') ? Number(searchParams.get('status')) : ''
        ...
        const totalRes = await Company.aggregate([
            {
                $match: filterObj
            },
            {
            ...
            {
                $match: { // Optional: Filter documents in the joined result
                "user.email": { $regex: '.*' + email + '.*' }
                }
            }, 
            { $count: "total" },            
        ])        
        const docRes = await Company.aggregate([
            {
                $match: filterObj
            },
            {
